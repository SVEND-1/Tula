...
interface Props {
    animal: Animal;
    variant: 'current' | 'next'; // current — активная карточка, next — под ней
    swipeClass?: string;          // анимация свайпа (только для current)
    getAnimalImage: (animal: Animal) => string | null;
    getAgeText: (age: number) => string;
    getGenderIcon: (gender: string) => string;
    getStatusLabel: (status: string) => string;
    truncateText: (text: string) => string;
    onNavigate?: () => void;      // кнопка "Подробнее" (только для current)
}
    ...
    return (
        <div className={`card ${variant} ${swipeClass}`}>
            <div className="card-image">
                ...
                {/* Бейдж статуса — только на текущей карточке */}
                {isCurrent && (
            ...
            <div className="card-info">
                ...
                {/* Кнопка "Подробнее" — только на текущей карточке */}
                {isCurrent && onNavigate && (
                    <button onClick={onNavigate} className="details-btn">
                        Подробнее
                    </button>
                )}
            </div>
